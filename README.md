...
<script>
    ...
    // تابع عادی‌سازی نام پروتکل برای تطابق با stats.json
    // ورودی: نام پروتکل از metadata (مثل 'ss', 'hysteria2', 'wireguard')
    // خروجی: کلید معادل در stats.json
    function getStatsKey(proto) {
        const lower = proto.toLowerCase();
        if (lower === 'ss') return 'shadowsocks';
        // سایر پروتکل‌ها دقیقاً به همان نام (با حروف کوچک) استفاده می‌شوند
        // برای اطمینان از اینکه hysteria2 و wireguard با حروف کوچک هستند
        return lower;
    }

    // دریافت درصد سلامت با استفاده از نگاشت
    function getHealthPercent(proto, stats) {
        const key = getStatsKey(proto);
        const healthValue = stats[key]?.health_sample?.percent;
        if (healthValue !== undefined && !isNaN(healthValue)) {
            return parseFloat(healthValue).toFixed(1);
        }
        // اگر کلید پیدا نشد، تلاش با حروف بزرگ (برخی stats ممکن است با حروف بزرگ ذخیره شده باشند)
        const upperKey = proto.toUpperCase();
        const healthValueUpper = stats[upperKey]?.health_sample?.percent;
        if (healthValueUpper !== undefined && !isNaN(healthValueUpper)) {
            return parseFloat(healthValueUpper).toFixed(1);
        }
        return '0.0';
    }
    ...
    async function loadDashboard() {
        ...
        try {
            ...
            for (const [proto, data] of protocols) {
                ...
                // نام نمایشی زیبا برای پروتکل
                let displayName = proto.toUpperCase();
                if (proto.toLowerCase() === 'ss') displayName = 'SS';
                else if (proto.toLowerCase() === 'shadowsocks') displayName = 'SS';
                else if (proto.toLowerCase() === 'hysteria2') displayName = 'HYSTERIA2';
                else if (proto.toLowerCase() === 'wireguard') displayName = 'WIREGUARD';
                else displayName = proto.toUpperCase();
            ...
            // رسم نمودار سلامت با استفاده از نگاشت صحیح
            const ctx = document.getElementById('healthChart').getContext('2d');
            const labels = [];
            const healthData = [];
            for (const [proto, data] of protocols) {
                const key = getStatsKey(proto);
                // همچنین سعی کنید با حروف بزرگ اگر پیدا نشد
                let healthVal = stats[key]?.health_sample?.percent;
                if (healthVal === undefined) {
                    healthVal = stats[proto.toUpperCase()]?.health_sample?.percent;
                }
                if (healthVal !== undefined && !isNaN(healthVal)) {
                    let displayLabel = proto.toUpperCase();
                    if (proto.toLowerCase() === 'ss') displayLabel = 'SS';
                    else if (proto.toLowerCase() === 'shadowsocks') displayLabel = 'SS';
                    labels.push(displayLabel);
                    healthData.push(parseFloat(healthVal).toFixed(1));
                }
            }
            ...
            if (labels.length > 0) {
                ...
                ctx.canvas.parentElement.innerHTML = '<div class="loading">داده‌ای برای نمایش نمودار وجود ندارد</div>';
            }
        } catch (err) {
            console.error(err);
